...
namespace WebBookmarkService.DAL
{
	public partial class UserConfigurationDAL
	{
        ...
        {
            string sql = "SELECT * FROM tblUserConfiguration WHERE UserConfigurationKey = @UserConfigurationKey and UserInfoID=@UserInfoID";
            using (MySqlDataReader reader = MyDBHelper.ExecuteDataReader(sql, new MySqlParameter("@UserInfoID", userInfoID),
                new MySqlParameter("@UserConfigurationKey", userConfigurationKey)))
            {
                if (reader.Read())
                {
                    return ToModel(reader);
