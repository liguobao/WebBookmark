...
namespace WebBookmarkService.DAL
{
	public partial class UserWebBookmarkImportLogDAL
	{
        ...
        /// <returns></returns>
        public IEnumerable<UserWebBookmarkImportLog> LoadByUID(long uid)
        {
            string sql = "SELECT * FROM tblUserWebBookmarkImportLog where userInfoID = " + uid + " order by CreateTime desc";
            using (MySqlDataReader reader = MyDBHelper.ExecuteDataReader(sql))
            {
                return ToModels(reader);
            }
