...
namespace WebfolderService.DAL
{
	public partial class UserWebFolderImportLogDAL
	{
        ...
        /// <returns></returns>
        public IEnumerable<UserWebFolderImportLog> LoadByUID(long uid)
        {
            string sql = "SELECT * FROM tblUserWebFolderImportLog where userInfoID = " + uid + " order by CreateTime desc";
            using (MySqlDataReader reader = MyDBHelper.ExecuteDataReader(sql))
            {
                return ToModels(reader);
            }
